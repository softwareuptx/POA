...
class Unidades extends CI_Controller
{
    ...
    public function editar($id=NULL)
    {
        ...
        $this->form_validation->set_rules('responsable', 'Nombre del responsable', 'required|callback_validarpersona');

        if( $this->form_validation->run() && $this->input->post() )
        {   
            $unidad       = $this->munidades->obtener($id);
            $responsable  = $this->input->post('responsable');

            //Si el responsable es diferente isnertammos una nueva persona
            if($unidad->u_refsii!=$responsable){
                ...
                $this->munidades->eliminar_usuario($unidad->uni_responsable);
    ...
    public function eliminar($id=NULL)
    {
        ...
        if(!$id)
            $this->alerts->_403();
        //Validos la informacion
        if(!$unidad=$this->munidades->obtener($id))
            $this->alerts->danger('unidades',$this->alerts->db_404);

        //Validamos si la operacion de realizo con éxito
        if($this->munidades->eliminar($unidad))
        {
    ...
    {
        if($this->mpersonas->obtener_refsii($id))
        {
            $this->form_validation->set_message('validarresponsable', 'Esta persona ya tiene una aŕea a su cargo, por favor selecione otro responsable');
            return FALSE;
        }
        return TRUE; 
    }
